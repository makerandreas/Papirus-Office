...
**Date:** 2026-09-24 (v2.1 — all ⚑ items adjusted, see §7.12)
...
**Baseline:** `main` `55a9a97` (PR #12 merged), branch convention `arena/<session>-papirus-office`, CI is the only compile/test evidence (no local JDK; `./gradlew testDebugUnitTest` + the nightly build job in `.github/workflows/build.yml`).
**Relationship to plan 1:** `plan-01-master-index.md` keeps the WG-chapter mapping, the checklist mapping and the finding registry; this file keeps the PR order and the per-PR scope. Plan 1's update rule (§6) is executed per PR as tabulated in §4.
**Evidence base:** every plan and audit in `anti-slop/` re-read; all 12 `tests/inky` files re-unpacked and re-inventoried (§2); the ODF 1.4 Part 3 schema in `docs/html` probed for every element a parser must read; ECMA-376 §17.9.18 (`w:numId=0`) and the Open XML SDK WordprocessingML references checked online; LibreOffice Writer Guide 24.8 Chapter 1 read in full (all 8 sections of the online chapter) and re-mapped in Appendix A. Every file:line citation below was re-opened in the working tree at `55a9a97` on 2026-09-24.
...
| ODT TOC = **authored snapshot** (no "Update Index" stub); regeneration is a later feature after a field model exists | PR 19 G-2; PR 21's DOCX TOC handling mirrors it (the DOCX samples carry the same authored snapshots, §2.3). |
...
**New in v2, needing no re-ask because they are recorded here for the user to strike:** items ⚑3.14 and ⚑3.15 (PR 13: Navigator category honesty, status-bar object info), ⚑G-4b (PR 18/20: bookmark parsing) and ⚑G-7 (PR 19/21: section identity). Each is additive, small, and unblocks an existing checklist item or a verified sample fact rather than inventing a feature.
...
`SourceHygieneGuardTest` records the tolerated raw-grey allowances at guard birth (47 across 14 files):

| File | Grey allowance |
|---|---|
| `modules/slidia/SlidiaModule.kt` | 8 |
| `ui/components/SwTextFormattingInspectorDialog.kt` | 7 |
| `modules/cellina/CellinaModule.kt` | 7 |
| `modules/pagella/PagellaModule.kt` | 4 |
| `modules/inky/HomeSubpages.kt` (document-colour palette, plan-03 3.11) | 4 |
| `ui/components/UniversalNavigatorSheet.kt` | 3 |
| `modules/inky/InkyModule.kt` | 3 |
| `ui/components/UniversalOdfSheet.kt` / `UniversalEmailSheet.kt` / `UniversalClipboardSheet.kt` / `OfficeUiComponents.kt` | 2 each |
| `ui/components/UniversalChartSheet.kt` / `CloudSyncBar.kt` / `LayoutDrivenDocumentRenderer.kt` | 1 each |

The guard's own header says plan 3B owns burning them to zero. PR 13 scope below includes deleting the map entries as the allowances reach zero.
...
| Ribbon: 8 tabs declared, only File+Home have content, literal "…will be implemented soon." | ✅ `InkyModule.kt:2936`, `:3381-3390` | 13 (3B) |
| Toolbar hub image/table/link/comment toast-only (TODOs landed in 12) | ✅ `InkyModule.kt:2790-2811` | 13 (3B) |
| ⚑ Navigator: 13 categories rendered, ~20 `EmptyCategoryRow` stubs | ✅ `UniversalNavigatorSheet.kt` (categories: bookmarks, comments, fields, footnotes, frames, headings, images, ole, pages, sections, shapes, tables; no "hyperlinks" category) | 13 (3B) + 15/17/18/19/20/21 fill |
| ⚑ No bookmark parsing anywhere (tokens exist, unconsumed; DOCX `w:bookmarkStart/End` unhandled) | ✅ `OdfXmlToken.kt:39-41` has `XML_BOOKMARK*`, no context arm; no `bookmark` in `OfficeDocumentParser.kt` | 18 (ODF) / 20 (DOCX) |
...
## 2. Sample matrix (re-inventoried this session, raw XML in the unpacked copies)

Method: exact-tag-boundary counts (v1's "6 TOC instances" for Sample-2 was a prefix-grep artifact — `<text:table-of-content` also matches `-source` and `-entry-template`; the corrected numbers are below).

### 2.1 ODT (`content.xml` + `styles.xml`)

| # | text:h | text:p | lists (items) | list-style defs (content/styles.xml) | tables | TOC (entries/links) | sections | frames/images | text:a | soft-page-break |
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | 28 | 183 | 38 (84) | 0 / 11 (`WWNum1…11`) | 3 | – | – | 6 / 6 | 0 | 8 |
| 2 | 30 | 308 | 11 (103) | 0 / 26 | 2 | **1** (15 / 15) | – | 11 / 10 | 15 | 10 |
| 3 | 0 | 170 | 0 | 0 / 0 | 1 | – | – | 0 / 0 | 0 | 0 |
| 4 | 7 | 112 | 1 (18) | 0 / 5 | 0 | 1 (22 / 21) | – | 1 / 1 | 23 | 2 |
| 5 | 0 (headings are styled `text:p`) | 218 | 79 (186) | 0 / 18 | 0 | 1 (15 / 14) | – | 2 / 1 | 14 | 2 |
| 6 | 45 (6/11/28 at levels 1/2/3) | 322 | 60 (180) | **19 / 1** (`Makalah_20_Default` in styles.xml) | 1 | 1 (45 / 45) | 5 | 3 / 3 | 45 | 0 |

Fidelity references: Sample-6's chapter numbering `Makalah_20_Default` (level-1 `num-prefix="BAB "`, `display-levels`) lives **only in `styles.xml`**; its 19 siblings are automatic styles in `content.xml`. `Subbab` declares level 3 with `style:num-format=""` (the "no number" edge). The Sample-6 TOC carries roman page numbers (ii/iii) for the front matter and `text:a "#_TOCxxxxx"` anchors. Sample-5's page style is A4 with `margin-top 0cm`/`margin-bottom 1cm`; its default paragraph is Aptos 12 pt.
...
| # | ¶ | tbl (rows,cols) | drawings | hyperlinks | sectPr | numPr | TOC snapshot (toc-styled entries) | instrText | fldSimple |
|---|---|---|---|---|---|---|---|---|---|
| 1 | 211 | 3 (57,57) | 6 | 0 | 1 | 54 | – | 9 (SEQ only) | 0 |
| 2 | 351 | 2 (38,57) | 12 | 15 | 5 | 106 | 15 (6+9) | 14 (TOC+SEQ) | 0 |
| 3 | 169 | 1 (24,40) | 0 | 0 | 1 | **0** | – | 0 | 0 |
| 4 | 116 | 0 | 1 | 23 | 6 | 17 | 21 (7+14) | 1 | 0 |
| 5 | 217 | 0 | 1 | 14 | 5 | 75 | 14 (6+8) | 1 | 0 |
| 6 | 371 | 1 (18,30) | 3 | 45 | 5 | 133 | **45 (6+11+28)** | 1 (TOC) | 0 |

The TOC snapshots are authored `toc 1/2/3`-styled paragraphs (the DOCX twins of the ODT `text:table-of-content` bodies, §2.1). Sample-6's 45 entries split 6/11/28 by level — the same distribution as its 45 headings. Parsing gotchas verified: Sample-4's field instruction arrives XML-entity-escaped (` TOC \o &quot;1 - 2&quot; \z `), Sample-4 defines unused `toc 3…9` + `TOC Heading` styles (a name-matcher must not treat style *definition* as style *usage*), and `w:hyperlink` counts exceed TOC entries in Sample-4 (23 total, 21 in the TOC).

Cross-format pair check: ODT `text:a` counts equal DOCX `w:hyperlink` counts per sample (15/23/14/45) — the pairs are the same documents. No `gridSpan`/`vMerge` in any sample, so merged cells stay deprioritised (parse `tblHeader`, render, but do not build the merge model yet).

### 2.3 ⚑ The Sample-6 DOCX heading architecture (new, verified this session)

This is the single most important structural fact the DOCX plans must encode, and it corrects v1's assumption that "headings carry no rFonts/sz and the char styles are orphaned." The full picture:

1. **The heading *styles* carry the numbering.** `para1…para9` are named `heading 1…heading 9`, `basedOn para0` (Normal: Times New Roman, `w:lang id-id`). Their `w:pPr` contains `w:numPr` → `w:ilvl N-1` / **`w:numId 15`**, plus `jc=center`, `keepNext`, `keepLines`, `outlineLvl=N-1`; their `w:rPr` is only `w:b`/`w:bCs`. `w:num 15` → `abstractNum 15`: `lvlText` = `BAB %1`, `%1.%2`, `%1.%2.%3`, … `start=1`.
2. **The heading *character* styles are linked by naming convention, not by `w:link`.** `char1…char9` are named `Heading 1 Char…Heading 9 Char` (`w:customStyle="1"`, `basedOn char0` = "Default Paragraph Font"). **There is no `w:link` attribute anywhere in the file.** Word's built-in convention pairs a paragraph style named `X` with a character style named `X Char`; the engine must implement both mechanisms (explicit `w:link` first, then the name convention). Verified metrics: char1 = Aptos Display 20 pt (sz 40) `#0f4761`; char2 = Aptos Display 16 pt (sz 32) `#0f4761`; char3 = 14 pt (sz 28) `#0f4761` with no own `rFonts` (inherited up the chain).
3. **Paragraphs override the style's numbering with direct `w:numPr`.** All 45 heading paragraphs use `pStyle para1/2/3` (6/11/28 — the same distribution as the ODT's 45 headings at levels 1/2/3). **42** carry a direct `w:numPr` repeating the style's `numId=15`; **3** carry **`w:numId 0`** — KATA PENGANTAR and DAFTAR ISI (and one more) — which per ECMA-376 §17.9.18 *"shall never be used to point to a numbering definition instance, and shall instead only be used to designate the removal of numbering properties"*. Those three headings must render **without** the `BAB N` prefix; the other 42 must render **with** it, computed from the style-inherited numbering.
...
5. **The DOCX TOC is an authored snapshot** (same decision as ODT). Entries are plain paragraphs styled `para16/17/18` = **`toc 1/2/3`** (basedOn Normal), each a `w:hyperlink w:anchor="_TOCxxxxx"` containing the entry text, a `w:tab`, and the literal page number; the style carries `w:tabs` with a right-aligned tab at 9027 twips, `leader="dot"`. The field wrapper's instruction is ` TOC \o "1 - 9" \z `; bookmark `_TOC000009` sits in the real PEMBAHASAN heading.
6. **Sections carry page-numbering formats.** The five `w:sectPr` share geometry but differ in `w:pgNumType`: §1 `lowerRoman start=1 titlePg` (front matter, roman ii/iii — the same roman pages the ODT TOC shows), §2 `decimal start=1` (body restarts), §3–5 `decimal`. The status bar must be able to tell **page number** from **sequence number** (WG Ch.1 status-bar item), so the section model needs `pgNumType` (fmt/start) and `titlePg`, not just geometry.

**Consequences for the plans:** PR 20 must resolve the full style chain *including* naming-convention char links and must surface, per heading paragraph, whether numbering is inherited, explicit, or suppressed (`numId=0`) — as a model fact PR 21 renders. PR 21 renders the `BAB N` labels from `abstractNum 15`, handles the three suppressed headings, and keeps the TOC snapshot intact (toc styles, dot leaders, literal pages, `_TOC` anchors). PR 22's writer must reproduce a `styles.xml` with the heading/char pairs and a `numbering.xml` the generated `document.xml` actually references.

### 2.4 What the matrix dictates for the plans (v1 items, corrected + new)
...
2. **The numbering model tolerates empty `style:num-format`** (Sample-6 `Subbab`) as "no number".
...
5. **The heaviest TOC case is Sample-6** (45 entries/45 links, 19.8 KB of index body), then Sample-4 (22/21) and Sample-2 (15/15) — not Sample-2 "six instances" as v1 stated.
...
| 10 | — | Font engine + design language — **parked** | 21 + user decision | — | resume trigger §4.12 |
...
**Goal:** nothing in the UI pretends. Closes plan-03 3.5, 3.6, 3.7, 3.9, 3.10, 3.11, 3.28, plus the three ⚑ items from this session's verification.

**Scope**

1. **3.6 Ribbon.** Rebuild the tab list from `CONCEPT.md` (File, Home, Insert, Layout, Review, View + contextual tabs when their context exists); settle `References`/`Mailings`: **drop both** until their features exist — `CONCEPT.md` does not list them for Writer, and a permanently disabled tab is the louder lie. Unimplemented tabs render disabled with an accessible reason; a press raises the honest note as a string resource (replacing the `InkyModule.kt:3383` literal).
2. **3.5 Hub tools.** The four toast-only tools (image, table, link, comment) get the visible "coming in a later build" state; the `TODO(plan-6/7/8)` markers from PR 12 already sit on them.
3. **3.7/3.28 Drive.** Keep the honest placeholder; correct `AGENTS.md` + `PROJECT_CONTEXT.md` to "placeholder, not yet implemented".
4. **3.9/3.10 Tokens and targets outside Inky.** `CellinaModule.kt:800,811`, `SlidiaModule.kt:904,915`, `PagellaModule.kt:106,110` zoom pairs → 48 dp targets; the raw greys in those modules, `HomeDashboard`, `SwTextFormattingInspectorDialog`, `UniversalNavigatorSheet`, `CloudSyncBar`, `UniversalChartSheet` and `LayoutDrivenDocumentRenderer.kt:665` (the `Color.DarkGray` cell text the code comment itself admits fails 4.5:1) move to the token set. **Every one of the 47 recorded grey allowances in `SourceHygieneGuardTest` burns to zero and the map entries are deleted** (the guard's header assigns this to 3B; paper stays white/black — that is document, not chrome).
5. **3.11 Palette.** Keep the Material-2014 hues as document colours; one `antislop-code`-compliant comment recording the decision.
6. **⚑ 3.14 Navigator categories (new).** `UniversalNavigatorSheet` renders 13 categories with ~20 `EmptyCategoryRow` stubs and no "hyperlinks" category, while WG Ch.1 and checklist item 7 expect the Navigator to list what the document contains. In this PR only: each stub becomes one of **two** honest states, chosen by whether the model can yet *see* that object class — **verified-absent**: "No %s in this document", only for classes the parser fully reads today (headings, tables, images, pages — verified at `55a9a97`: the index engine populates exactly these, and ODF frames produce no element of their own, so Text Frames is *not* readable); **not-yet-readable**: a disabled row "Not yet available in this build" with the owning-plan marker (`TODO(plan-19)` frames/sections, `TODO(plan-18/20)` hyperlinks/bookmarks, `TODO(plan-21)` fields, `TODO(unassigned)` comments/footnotes/ole/shapes) — a parser that cannot read footnotes may not claim the document has none. The **Hyperlinks** category (checklist item 7: "Images and Hyperlinks categories must exist") is added in the not-yet-readable shape; the Navigate-By filter option for it lands with the data (18/20), so no dead filter entry is added. No jump behaviour in this PR — the data lands with the owning plans.
7. **⚑ 3.15 Status-bar object information (new, WG Table 1).** WG Ch.1 gives the status bar a "section or object information" field (image/frame → size+position; list item → level+list style; heading → heading level; table → name+cell reference; section → name; index → type). Minimal honest version here, with the data sources fixed: the existing caret-to-element mapping (`DocumentTextWindows.elementForOffset`, plan 2 infrastructure) yields — cursor in a heading: "Heading N: text"; cursor in a table: "Table" (the mapping resolves caret to element, not to cell, so row/col precision is *not* invented here — it lands with table geometry in 19/21); anything else: a hyphen placeholder (`"-"`, never an em dash, R-02). The slot and string resources are created here; sections, frames and indexes fill the same slot after 19/21 when those models exist.

**Files:** `InkyModule.kt` (ribbon/hub/status-bar regions), `UniversalNavigatorSheet.kt`, the three other module files, `HomeDashboard.kt`, `SwTextFormattingInspectorDialog.kt`, `CloudSyncBar.kt`, `UniversalChartSheet.kt`, `LayoutDrivenDocumentRenderer.kt` (cell colour only), `strings.xml`, `SourceHygieneGuardTest.kt` (allowances → 0), `AGENTS.md`, `PROJECT_CONTEXT.md`.
**Tests:** Robolectric: disabled tabs expose `isEnabled=false` + semantics; hub tools assert their disabled state; every Navigator category row is either populated or an honest empty state (no bare `EmptyCategoryRow` with no content description); the guard's grey allowances are zero and the map is empty.
...
6. **E-7 · One render transform (+ ⚑ centralisation).** The renderer derives card size, margins, and text scale from the page's own `widthDp/heightDp` through a single `pageScale`; `PageStackMetrics.BASE_CARD_WIDTH_DP` retires from the render path. ⚑ The status bar's page-range logic currently re-implements the same fit-scale computation inline (`InkyModule.kt:2412-2431`) — it consumes the same single transform as the renderer, so the two can never drift again. `PageStackMetrics` survives only as the Go-to-Page/visibility-range input until that range derives from real page heights.
...
1. **G-1:** tokens + parser for `text:list-style` and its level styles from **both** `content.xml` and `styles.xml` (§2.4.1), including empty `style:num-format` (§2.4.2), `num-prefix/suffix`, `display-levels`, `start-value`, `bullet-char`, per-level text/paragraph properties, `text:continue-numbering` (35 uses in Sample-6), and resolution via paragraph auto-styles (`P5` → `Makalah_20_Default`). Output through E-EN-4's `NumberingSpec`; `CounterState` renders `BAB 1`, `2.1`, `a.`, `•`. The hard-coded `• `/`◦ ` dies. Sample-5's headings-as-paragraphs keep working (§2.4.6).
...
4. **⚑ G-4b (new) · Bookmarks.** `XML_BOOKMARK`/`-START`/`-END` arms (tokens already exist, unconsumed) into a lightweight `OfficeBookmark` (name, anchor position) feeding the Navigator's bookmarks category (§1.3) and the `_TOCxxxxx` anchors the ODT TOC snapshots reference (Sample-6: 92 bookmark-starts). The DOCX arm (`w:bookmarkStart/End`, 46 in Sample-6) lands in PR 20 alongside the style-chain work — same file, same pass — so both formats expose anchors to the TOC snapshot rendering (19/21) and the Navigator stub (13) in the same PR wave.
...
**Tests:** `OdtListNumberingTest` (Sample-6: `BAB`-shaped labels with per-chapter restarts; `Subbab` level 3 renders no number; Sample-1/2/4/5 labels present and Sample-3 unchanged), `HyperlinkFidelityTest` (Samples 2/4/5/6 text counts), `HeadingRunsTest`, `OdtBookmarkTest` (Sample-6: 92 named anchors survive).
...
3. **⚑ H-4b (new) · TOC snapshot, DOCX half.** The authored TOC renders as the ODT half does (G-2): entries styled `toc 1/2/3` with their `w:tabs` right-aligned dot-leader tab (Sample-6: pos 9027, `leader=dot`), literal page numbers, and `w:hyperlink w:anchor="_TOCxxxxx"` preserved as anchors (bookmarks from PR 20's DOCX arm). The ` TOC \o "1 - 9" \z ` instruction never appears in output. Entry detection is by **toc-styled paragraph, regardless of whether it sits inside a field-result run** (the samples author the snapshot as plain paragraphs). The test asserts the verified sizes (Sample-2: 15, Sample-4: 21, Sample-5: 14, Sample-6: 45; §2.2), handles the `&quot;`-escaped field instruction (Sample-4), and does not list the unused `toc 3…9`/`TOC Heading` *style definitions* (Sample-4) as entries.
...
|---|---|---|
| 13 | row 3 → "3A landed (PR 12); 3B landed (PR 13)" | plan-03 §8 gets a 3B record (3.5–3.11, 3.28, ⚑3.14/⚑3.15 as shipped) |
...
One-time plan-1 changes this session (land with the PR 13 commit, since they describe state now): registry row 3 status (3A **landed**, not "in review"); §3 WG-mapping gains three owner lines (Navigator categories → 13/18/19/20/21; status-bar object info → 13 + 19/21; bookmarks → 18/20); §6's "Next actions" item 2 is struck through as done.
...
3. **⚑ Sample-6 DOCX heading architecture added (§2.3)** — v1 said the heading runs "lack rFonts/sz" and left the `w:link` question open. Verified reality: the *styles* carry the numbering (`numId 15`, `BAB %1`), the *char styles* link by naming convention (no `w:link` in the file), 42 headings carry explicit `numPr` and 3 carry `numId=0` suppression, page breaks sit inside heading paragraphs, and the TOC is an authored snapshot of `toc 1/2/3`-styled hyperlink paragraphs. This grows PR 20 (H-1b, H-1c, H-2b) and PR 21 (⚑H-3 style-inherited numbering, ⚑H-4b TOC snapshot, ⚑H-6b pgNumType).
4. **⚑ Navigator honesty item (3.14)** — 13 categories with ~20 `EmptyCategoryRow` stubs and no Hyperlinks category were not in v1's PR 13; checklist item 7 makes the Hyperlinks category mandatory, so it is added here.
5. **⚑ Status-bar object information (3.15)** — WG Ch.1's "section or object information" field (Table 1) had no owner; the honest-minimal version lands in 13, the full version after 19/21.
6. **⚑ Bookmarks (G-4b in 18, DOCX arm in 20)** — tokens exist but are unconsumed; the Navigator bookmarks category and the `_TOC` anchors need them.
7. **⚑ Sections (G-7 in 19, H-6b in 21)** — ODT `text:section` identity had no plan item; DOCX `pgNumType`/`titlePg` is now explicit (WG status-bar page-number-vs-sequence-number).
8. **⚑ E-7 centralisation (15)** — the status bar's duplicated fit-scale computation now consumes the renderer's single transform.
9. **⚑ E-2 line-height constant (16)** — the renderer's `(sizeSp + 5f)` magic line height dies with the fudge, in favour of style-driven line height.
10. **Guard baseline made explicit (§1.2)** — the 47 grey allowances are tabulated; PR 13's scope now states that the map entries are deleted at zero.
11. **`w:br` handling corrected in the record** — the shared parse block *does* distinguish `type="page"` from soft breaks (`OfficeDocumentParser.kt:961`); v1's "still open" row implied otherwise. The remaining issue is `w:lastRenderedPageBreak` (ODT soft-page-break pollution), unchanged.
12. **⚑ items adjusted (v2.1, 2026-09-24):** on the user's instruction all ⚑ items were re-examined and tightened — **⚑3.14** now distinguishes *verified-absent* ("None in this document", only for classes the parser fully reads) from *not-yet-readable* (disabled "Not yet available in this build" + plan marker), because a parser that cannot read footnotes may not claim the document has none; **⚑3.15** fixes the data sources (caret→element mapping; table row/col; heading level+text; hyphen fallback, no invented data); **⚑G-4b** pins the DOCX `w:bookmarkStart/End` arm to PR 20 so both formats expose anchors in the same wave as the TOC snapshot rendering; **⚑G-7** leaves the section model shape (wrapper vs attribute) to implementation, fixed on the exposed name; **⚑H-1b** adds the precedence (explicit `w:link` → naming convention → style's own `rPr`; a direct run always overrides); **⚑H-1c** states the three-state flag and that the `pStyle` chain is walked through `w:basedOn`; **⚑H-2b** fixes the fallback for styleIds missing from the file's own style table (Normal-based, not a heading); **⚑H-3** states the resolution precedence (direct `numPr` incl. `numId=0` → style chain → none); **⚑H-4b** fixes entry detection to "toc-styled paragraph, inside or outside a field-result run"; **⚑H-6b** records `headerReference`/`footerReference` in the model without rendering them (no plan item owns page furniture).
...
5. **⚑ The new scope items (⚑3.14, ⚑3.15 in PR 13; ⚑G-4b bookmarks in PR 18/20; ⚑G-7 sections in PR 19/21; ⚑H-1b/H-1c/H-2b and ⚑H-3/H-4b/H-6b in PR 20/21)** are additive and each unblocks an existing checklist item or a verified sample fact; strike any of them and the owning PR simply drops that bullet — nothing else depends on them.
...
- Char style: `<w:style w:type="character" w:styleId="char1" w:customStyle="1"><w:name w:val="Heading 1 Char"/><w:basedOn w:val="char0"/><w:rPr><w:rFonts w:ascii="Aptos Display" …/><w:color w:val="0f4761"/><w:sz w:val="40"/>…` — **no `w:link` attribute exists anywhere in the file** (regex over all 37 styles: zero matches).
...
- TOC entry: `<w:p><w:pPr><w:pStyle w:val="para16"/>(= "toc 1")<w:tabs …><w:tab w:val="right" w:pos="9027" w:leader="dot"/></w:tabs>…<w:hyperlink w:anchor="_TOC000009"><w:r><w:t>BAB 2  PEMBAHASAN</w:t><w:tab/><w:t>4</w:t></w:r></w:hyperlink></w:p>`
- Sections: `sectPr1: pgNumType=lowerRoman start=1 titlePg; sectPr2: decimal start=1; sectPr3-5: decimal`.
- `abstractNum 15`: `lvlText` = `BAB %1`, `%1.%2`, `%1.%2.%3`, … `start=1`.
- ECMA-376 §17.9.18 (verified online, ECMA-376 numbering part): *"A value of 0 for the val attribute [of `w:numId`] shall never be used to point to a numbering definition instance, and shall instead only be used to designate the removal of numbering properties at a particular level in the style hierarchy."*

**ODF (local `docs/html/OpenDocument-v1.4-part3-schema.html`, authoritative per `AGENTS.md`):** tokens confirmed present in the schema for every element the plans parse — `text:list-style` (17 refs), `text:list-level-style-number` (25), `text:display-levels` (9), `text:table-of-content` (79), `text:index-body` (50), `text:continue-numbering` (9), `style:table-column-properties` (13), `style:column-width` (6), `text:numbered-paragraph` (32), `text:bookmark` (88), `fo:break-before` (11), `widows` (13), `orphans` (10).

**Engine (file:line at `55a9a97`):** `LayoutEngine.kt` — `elementGapDp` :107, `StyleResolver` 14 sp :61, `2.5f` fudge :154, table `rows*35+10`; `LayoutDrivenDocumentRenderer.kt` — `PageStackMetrics` (320/452) :56-61, `fitScale`/`renderScale` :149-150, `pageScale` :243-245, line height `(sizeSp+5f)` :474/:500/:579/:596/:650, `Color.DarkGray` cell :665; `InkyModule.kt` — `isWebView` :122, cursor-ratio page estimate :524-533, delays :1105-1109, `docxExtents` :212, hub tools :2790-2811, status bar (page range + fit-scale re-implementation) :2396-2470, ribbon tabs :2936, unimplemented-tab literal :3383; `OfficeDocumentParser.kt` — `extractDocxPageStyleSpec` :144, `extractDocxStyles` :218 (names only, `namespaceAware=false`), regexes :55-57, `currentRuns` :800, `w:br` type distinction :961-971, `w:tab` :958; `DocxDocumentParser.kt` — `[Image:]` :723, hard-coded `w:numId 1` :740, `generateOdtXml` (`office:version="1.2"`) :748+; `OdfXmlToken.kt` — `XML_A` :24, `XML_BOOKMARK*` :39-41, no list-style/index tokens; `SvXMLImportContext.kt` — hard-coded bullets, no `XML_A`/bookmark arms, soft-page-break mid-paragraph.

**Navigator (⚑):** `UniversalNavigatorSheet.kt` categories = bookmarks, comments, fields, footnotes, frames, headings, images, ole, pages, sections, shapes, tables; ~20 `EmptyCategoryRow` stubs; no hyperlinks category.

**Strings/guard:** `values/strings.xml` 701 keys; `values-in/strings.xml` 27 keys (frozen); `SourceHygieneGuardTest` — 47 grey allowances/14 files, 7 banned literals, five active rules (literal toasts, literal `contentDescription`, raw greys above allowance, em dashes in literals, resurrected 3.3 literals), comment-masking self-test.
...
**WG 24.8 Chapter 1 (read in full this session) — the behaviours the plans touch:** status bar fields (document-changes indicator *click-to-save*; page field with **Go to Page on click and bookmark list on right-click**; word/char count with **selection count temporarily replacing the total**; page-style field; text language; insert/overwrite mode; selection mode; digital signature; **section-or-object information** (Table 1: image/frame → size+position dialog; list item → level+list style; heading → numbering level; table → name+cell reference; section → name; index → type); view layout; zoom). Navigator (Navigate By **Page with sequence numbers — which differ from page numbers when numbering restarts**, double-click jump + highlight, content navigation view, heading-level filter, hidden items greyed with "hidden" tooltip, object renaming). Outline folding (Tools > Options > *Show outline-folding buttons* — the discoverability fix belongs to plan 3's InkyC1Checklist item, the app's double-tap is undocumented). Reminders (max 5, sixth evicts the first, **not saved with the document**, jumped via Navigate By). Undo/redo (list of sequential changes; the app's chevron removal was a user decision in audit-003 P1-2 and stands). Reload ("Cancel all changes?" confirmation). Close (save/discard prompt; last document → Start Center). Views (Normal/Web/Full Screen; **Web view allows zoom only** — the app's `isWebView` second renderer must be aligned or retired per plan-01 §3). New document (Start Center recents + templates; template dialog; the app's fake "RESUME (MODERN)" template content at `InkyModule.kt:1130` is a plan-3 honesty item). Save (Save/Save As/Save a Copy/Save All; **save the ODF original before saving a Word copy**; password/GPG and remote servers have no mobile analogue here — out of scope).
...
les/312 rows/1852 cells, 0 formulas; XLSX: 12 sheets, 48 shared strings, 2 formulas; ODP: 51 pages/506 text boxes; PPTX: 51 slides/996 shapes/55 media) as the floor for the Cellina/Slidia stages. They are referenced here only so the later-stage plans start from verified structure. Nothing in PRs 13–22 reads, tests or changes them.
