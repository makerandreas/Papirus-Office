...
6. **⚑ 3.14 Navigator categories (new).** `UniversalNavigatorSheet` renders 13 categories with ~20 `EmptyCategoryRow` stubs and no "hyperlinks" category, while WG Ch.1 and checklist item 7 expect the Navigator to list what the document contains. In this PR only: each stub becomes an honest state — a one-line "None in this document" when the model has no such objects, otherwise a disabled row with the owning-plan `TODO(plan-15/17/18/19/20/21)` marker from the fill schedule in §5; and the **Hyperlinks** category (checklist item 7: "Images and Hyperlinks categories must exist") is added as a stub in the same shape. No jump behaviour in this PR — the data lands with the owning plans.
7. **⚑ 3.15 Status-bar object information (new, WG Table 1).** WG Ch.1 gives the status bar a "section or object information" field (image/frame → size+position; list item → level+list style; heading → heading level; table → name+cell reference; section → name; index → type). Minimal honest version here, with the data sources fixed: the existing caret/selection-to-element mapping (plan 2 infrastructure) yields — inside a table cell: "Table · row r, col c"; cursor in a heading: "Heading N · text"; anything else: a hyphen placeholder (`"-"`, never an em dash, R-02) — no invented data. The slot and string resources are created here; sections, frames and indexes fill the same slot after 19/21 when those models exist.
