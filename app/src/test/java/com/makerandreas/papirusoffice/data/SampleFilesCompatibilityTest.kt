...
class SampleFilesCompatibilityTest {
    ...
    private fun assertParses(parser: OfficeDocumentParser, path: String) {
        ...
        val parsed = runBlocking {
            // Wait for the parser's terminal state; failures carry a reason,
            // successful documents never report isParsingFailed.
            val result = parser.parseDocumentAsync(file)
            val deadline = System.currentTimeMillis() + 30_000
            var doc = result.value
            while ((doc == null || doc.isParsingFailed) && System.currentTimeMillis() < deadline) {
                try {
                    Thread.sleep(50)
    ...
    fun xlsx_sample1_has_named_tables() {
        ...
        val parsed = runBlocking {
            val result = OfficeDocumentParser(context()).parseDocumentAsync(file)
            val deadline = System.currentTimeMillis() + 30_000
            var doc = result.value
            while ((doc == null || doc.isParsingFailed) && System.currentTimeMillis() < deadline) {
                try {
